...
test("模型可只提交可靠问题或草稿，但完全空输出不能成为成功结果", () => {
  ...
  assert.equal(modelInterviewOutputSchema.safeParse({ assistantText: "", question: null, draft: { title: "旧结构", brief: {} } }).success, false)
  ...
  assert.equal(modelInterviewOutputSchema.safeParse({ assistantText: "", question: null,
    draft: { title: "旧结构", markdown: genericMarkdown, brief: {} },
  }).success, false)
