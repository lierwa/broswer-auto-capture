...
import { planEvidenceSchema } from "./plan-evidence.js"
...
export const planProposalSchema = z.object({
  ...
  objectives: z.array(z.object({ objective: text, sourceIds: z.array(id).max(15), stepIds: z.array(stepId).min(1).max(40), explanation: text }).strict()).max(40),
  ...
  for (const step of value.steps) {
    ...
    const pending = value.gaps.some((gap) => gap.disposition === "blocking" && gap.stepIds.includes(step.id))
    if (step.kind !== "derive" && !step.sourceIds.length && !pending) ctx.addIssue({ code: "custom", message: "浏览器步骤必须绑定来源或待核验缺口" })
    seen.add(step.id)
  }
  for (const objective of value.objectives) {
    const pending = objective.stepIds.every((step) => value.gaps.some((gap) => gap.disposition === "blocking" && gap.stepIds.includes(step)))
    if (!objective.sourceIds.length && !pending) ctx.addIssue({ code: "custom", message: "计划目标必须绑定来源或待核验步骤" })
  }
...
export const planRecordSchema = z.object({
  ...
  requirement: requirementBriefSchema, evidence: planEvidenceSchema, evidenceDigest: z.string().length(64).nullable(),
  stage: z.enum(["assessing", "source_evidence", "drafting", "complete"]),
  status: z.enum(["generating", "ready", "blocked", "failed", "cancelled", "manual_required", "cleanup_required", "interrupted"]), sequence: index,
  createdAt: z.string().datetime(), updatedAt: z.string().datetime(), current: text, reason: text.nullable(),
...
export const executionRecordSchema = z.object({
  ...
  requirementVersion: z.number().int().positive(), requirementRevision: index,
...
export const planCommandSchema = z.discriminatedUnion("type", [
  z.object({ type: z.literal("generate"), requestId: id, requirementVersion: z.number().int().positive(), budgetCeiling: planBudgetSchema.optional(), stepBudgetLimits: stepBudgetLimitsSchema.optional() }).strict(),
  z.object({ type: z.literal("cancel_generation"), planId: id }).strict(),
  z.object({ type: z.literal("return_to_interview"), planId: id, requestId: id, expectedRevision: index }).strict(),
