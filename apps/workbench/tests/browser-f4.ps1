...
try {
  ...
  Assert-Page 'document.body.innerText.includes("已停止生成")' 'cancelled'
  Mode 'failed'
  Activate 'button' '重新制定计划'
  Assert-Page 'document.body.innerText.includes("生成未通过")' 'failed-retry-path'
  Mode 'normal'
  Activate 'button' '重新制定计划'
  ...
  Mode 'blocking'
  Assert-Page 'document.body.innerText.includes("需求或证据已变更 · 待复核")' 'source-version-refresh'
  Activate 'button' '重新制定计划'
  Assert-Page 'document.body.innerText.includes("证据缺口待处理") && document.body.innerText.includes("阻塞启动")' 'blocking-gap'
  Mode 'invalidate'
  Assert-Page 'document.body.innerText.includes("需求或证据已变更 · 待复核")' 'stale-plan'
