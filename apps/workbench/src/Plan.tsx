...
const runLabels = { queued: "已授权 · 排队中", running: "正在探索", awaiting_next_stage: "等待链路验证", interrupted: "运行已中断", cancelled: "已停止", stale: "授权绑定待复核", failed: "执行未完成", manual_required: "需要人工处理", cleanup_required: "会话待清理" }
...
function BudgetReview({ record }: { record: PlanRecord }) {
  ...
  return <div className="source-gaps"><h3>本次授权预算</h3><p>最多 {steps.reduce((n, step) => n + step.budget.maxCommands, 0)} 条浏览器命令 · {Math.round(steps.reduce((n, step) => n + step.budget.timeoutMs, 0) / 1000)} 秒 · {steps.reduce((n, step) => n + step.budget.maxModelCalls, 0)} 次首次探索模型调用</p><p>达到上限暂停并保留已完成内容和剩余范围；新增来源或扩大预算须重新制定并确认计划。登录、验证码与访问限制转人工。</p></div>
}
