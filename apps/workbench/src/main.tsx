    ...
    <main className="app-shell task-layout" data-theme={theme} data-sidebar={!compact && sidebarOpen}>
      ...
      <section className="workspace">
        <header className="topbar"><div className="task-title-group"><Tooltip content={sidebarOpen ? "收起任务列表" : "展开任务列表"}><IconButton variant="ghost" color="gray" aria-label={sidebarOpen ? "收起任务列表" : "展开任务列表"} onClick={() => setSidebarOpen(!sidebarOpen)}><PanelLeft size={18} /></IconButton></Tooltip><h1>{task?.title ?? "浏览器工作台"}</h1>{task && <Badge color="gray" variant="soft">{taskStatusLabels[task.status]}</Badge>}</div>
          <div className="status-group">{task && <TaskMenu task={task} model={model} />}<Tooltip content="模型设置"><IconButton variant="ghost" color="gray" aria-label="模型设置" onClick={() => setModelSettingsOpen(true)}><Settings size={17} /></IconButton></Tooltip><Tooltip content={theme === "dark" ? "切换为浅色主题" : "切换为深色主题"}><IconButton variant="ghost" color="gray" aria-label={theme === "dark" ? "切换为浅色主题" : "切换为深色主题"} onClick={() => setTheme(theme === "dark" ? "light" : "dark")}>{theme === "dark" ? <Sun size={17} /> : <Moon size={17} />}</IconButton></Tooltip></div>
        </header>
        ...
        {model.visited.map((id) => { const item = model.tasks.find((entry) => entry.id === id); return item && <TaskWorkspace key={id} task={item} visible={id === model.selected} theme={theme} otherRunning={running?.id !== id ? running : undefined} modelSettings={modelSettings} /> })}
      ...
      <ModelSettingsDialog open={modelSettingsOpen} onOpenChange={setModelSettingsOpen} client={modelSettings.client}
        {...(modelSettings.selection ? { value: modelSettings.selection } : {})} onChange={modelSettings.save} loading={modelSettings.loading}
        {...(modelSettings.error ? { errorMessage: modelSettings.error } : {})} appearance={theme} selectOnConnect />
    </main>
  </Theme>
}
