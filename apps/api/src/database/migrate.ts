...
export function migrate(connection: Database.Database) {
  ...
  if (version === 4) return
  if (version !== 0 && version !== 1 && version !== 2 && version !== 3) throw new Error("数据库版本高于当前程序，已停止启动以保护数据。")
  ...
  connection.transaction(() => {
    ...
    connection.exec(`CREATE TABLE researchRuns (id TEXT PRIMARY KEY, taskId TEXT NOT NULL REFERENCES tasks(id), body TEXT NOT NULL CHECK(json_valid(body)));
