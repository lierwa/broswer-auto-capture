...
export function planEvidenceOutputSchema() {
  const { $schema: _, ...schema } = z.toJSONSchema(planEvidenceDecisionSchema, { target: "draft-7", override: ({ jsonSchema }) => {
    for (const key of ["format", "pattern", "minLength", "maxLength", "minItems", "maxItems"]) delete jsonSchema[key]
  } })
  return schema
}

export function planEvidencePrompt(brief: RequirementBrief, record: PlanRecord, current: { id: string; text: string } | null) {
  return [
    "你负责为抓取计划核验真实来源证据。只输出结构化判断；宿主执行有界浏览器动作。用途 plan_evidence。",
    ...
    JSON.stringify({ brief, remainingDecisions: 16 - record.evidence.audits.length, evidence: { ...record.evidence, audits: undefined }, current }),
  ].join("\n\n")
}
