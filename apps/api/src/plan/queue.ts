...
export const pendingExecution = (record: ExecutionRecord) => ["queued", "running", "awaiting_next_stage", "interrupted", "manual_required", "cleanup_required", "drift_paused"].includes(record.status)
// 执行器必须返回明确的验证成果；单纯回调退出不代表链路通过。
export type PlanExecutor = (input: { plan: PlanRecord; execution: ExecutionRecord; browser: Pick<BrowserSession, "command" | "beginStep">; signal: AbortSignal; save: () => void }) => Promise<void | { verified: true; reason: string; completed?: boolean }>
export class ExecutionQueue {
  ...
  async tick() {
    ...
    try {
      ...
      record.status = "running"; record.reason = "正在执行当前授权范围。"; record.browserRunId = record.attempt ? randomUUID() : record.id; record.attempt++; this.repository.saveExecution(record)
  ...
  private async run(record: ExecutionRecord, plan: PlanRecord, signal: AbortSignal) {
    ...
    try {
      ...
      record.status = outcome?.completed ? record.capture?.coverage === "completed" ? "completed" : "partial" : "awaiting_next_stage"
      record.reason = outcome?.verified ? outcome.reason : "探索处理器已返回，等待后续链路验证与执行；尚无全量完成结论。"
    } catch (error) {
      ...
      if (error instanceof Error && ["browser_state_drift", "required_field_missing", "target_missing", "target_ambiguous", "page_evidence_incomplete"].includes(error instanceof BrowserError ? error.code : error.message)) record.status = "drift_paused"
      if (record.capture) { record.capture.coverage = "partial"; record.capture.gaps = ["运行在当前步骤停止，已保存记录与未执行输入保留。"] }
