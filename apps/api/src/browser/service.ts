...
import { BrowserHost, BrowserError, grantSchema, type BrowserGrant, type BrowserHelpObserver, type BrowserHelpState, type BrowserSession, type CommandExecutor } from "@browser-capture/browser"
...
export class BrowserService {
  ...
  async run<T>(input: unknown, work: (session: Pick<BrowserSession, "command" | "state" | "beginStep" | "activeElapsedMs" | "stepElapsedMs">,
    ...
    try {
      const result = await this.host.run(grant, (session, lifetime) => work({ command: (command, commandSignal) => { validate(); return session.command(command, commandSignal) },
        state: () => session.state(),
