...
export function remember(context: StepContext, raw: unknown) {
  ...
  for (const match of page.text.matchAll(/@e\d+\s+\w+\s+"((?:\\.|[^"\\])*)"/g)) context.values.add(JSON.parse(`"${match[1]}"`))
...
export async function explore(context: StepContext) {
  while (true) {
    ...
    const decision = await modelDecision({ factory: context.factory, schema: explorationDecisionSchema, prompt: prompt(context), record: context.record,
      purpose: "exploration", phase: "exploration", nodeId: null, signal: context.signal, save: context.save })
