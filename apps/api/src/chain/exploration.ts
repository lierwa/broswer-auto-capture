...
export interface StepContext {
  ...
  priorCandidate?: Pick<ChainRecord, "graph" | "sample" | "verification"> | null;
}
...
function prompt(context: StepContext) {
  return [
    ...
    "某些目录末页仍保留可点的下一页。对此可使用branch_page_changed：在click下一页后read，与前一次read的完整语义页面比较，变化走changed（继续循环），未变化走unchanged（finish）。提取及checkpoint应在点击前进行。verification选择实际观察到的末页输入并跑到finish；完整运行的末页指纹还要与本次独立末页验证相同。不能把任意一次点击无效果单独当全量完成。",
    ...
    JSON.stringify({ step: context.step, requirement: context.plan.requirement, mappings: context.plan.proposal!.fields, sources: context.plan.sources,
      known: [...context.known], values: [...context.values], current: context.current, upstream: context.rows.slice(0, 6),
      history: context.history, remaining: context.remaining(), rejected: context.rejected, previousFailure: context.lastFailure, priorCandidate: context.priorCandidate ?? null }),
  ].join("\n\n")
}
