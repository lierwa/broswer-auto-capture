...
try {
  const state = current.plan.snapshot(before.taskId), plan = state.records[0]!
  ...
  assert.doesNotThrow(() => validateProposal(plan.proposal, plan))
  ...
  process.stdout.write("PASS real plan evidence validation and durable queue reopen; no new model or browser calls\n")
