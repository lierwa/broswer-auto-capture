...
正式队列已接入 Sol/high 受控探索、动作 DSL 编译、LangGraph 节点运行和逐步骤换输入验证。普通节点无模型入口，显式 LLM 使用独立 Luna/medium 预算；SQLite v6 保存链路版本、输入输出、节点事件和调用审计。真实海尔目录链路已完成样本及新输入验证，详情采集在本步骤预算处停止，完整范围保留。此出口是代表窗口验证；全量、覆盖、恢复、复跑和修复仍按 F6 验收，具体证据见 PROGRESS、RESEARCH R-012。

下一全新 session 只实施 F6，直接使用保存项目本地目录；先核验现场和交接，不重复授权消费旧队列，不推送远程。下文 F1–F4 出口保留为阶段历史。
