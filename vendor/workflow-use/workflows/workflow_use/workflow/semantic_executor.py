...
class SemanticWorkflowExecutor:
	...
	def _select_element_by_position(
		...
		if position_hint == 'first' and matching_elements:
			...
			return selected_element
		elif position_hint and position_hint.isascii() and position_hint.isdigit():
		...
		# WHY: 显式位置不存在时不能点击另一项；只有未指定位置才保留上游默认行为。
		if position_hint:
			return None

		# No position hint - return first match (highest priority)
		if matching_elements:
			selected_text, selected_element, priority = matching_elements[0]
	...
	async def execute_click_step(self, step: ClickStep) -> ActionResult:
		...
		if position_hint:
			if step.target_text:
				position_element = self._find_element_by_pattern(
					step.target_text, position_hint, getattr(step, 'container_hint', None)
				)
			if position_element is None:
				return ActionResult(error='target_position_unavailable')
		...
		if not position_hint and hasattr(step, 'selectorStrategies') and step.selectorStrategies:
	...
	try {{
		...
		if hasattr(step, 'target_text') and step.target_text:
			...
			direct_selector = None if position_hint else await self._try_direct_selector(step.target_text)
			if direct_selector:
		...
		fallback_selectors = []
		if element_info and not position_hint:
		...
		async def click_executor():
			# WHY: 文本与宽泛 fallback 不保留 ordinal；复用上游 Element.click，且只接受唯一目标。
			if position_hint:
				elements = await self._get_elements_by_selector(selector_to_use)
				if len(elements) != 1:
					raise ValueError('target_position_unavailable')
				await elements[0].click()
				return ActionResult(extracted_content='Clicked positional target', include_in_memory=True)
	...
	async def execute_step(self, step: WorkflowStep) -> ActionResult:
		...
		if isinstance(step, NavigationStep):
			...
			# WHY: schema 的两种提取拼写共用既有实现，保留输出与验证元数据。
			adapted = ExtractStep.model_validate({
				**step.model_dump(exclude={'type', 'goal'}),
				'type': 'extract', 'extractionGoal': step.goal,
			})
			return await self.execute_extract_step(adapted)
