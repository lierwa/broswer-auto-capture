...
class StepVerifier:
	...
	async def verify_step(
		...
		if total_checks == 0:
			...
			if confidence >= 0.7 and step_type != 'hybrid_declared':
				result = VerificationResult.SUCCESS
			else:
				result = VerificationResult.FAILURE
	...
	def _get_verification_checks(self, step: Any, pre_state: Optional[Dict[str, Any]] = None) -> List[VerificationCheck]:
		...
		step_type = getattr(step, 'type', 'unknown')
		checks = []
		# WHY: hybrid 的已证明条件必须逐项执行；不从动作描述猜测效果或补充 AI 检查。
		if step_type == 'hybrid_declared':
			return step.verification_checks
	...
	async def _run_deterministic_check(
		...
		if check_fn == 'check_declared_fact':
			from workflow_use.hybrid.postconditions import check_fact
			return await check_fact(check.parameters, browser_session)
